...
(function (c, g) {
    ...
    var k = {
        ...
        "You can upload only files with extension .jpeg and .png.": 16,
        "File size should not exceed 5 MB.": 17
    }, r = {
        ...
        init: function (a, b, e, d, c) {
            ...
            var a = this.AU;
            if (a.hasHtml4) {
                var a = a.getParams(this.name, 0, !1), b = c('<form action="' + this.settings.url + '" method="post" target="ax-main-frame" encType="multipart/form-data" />').hide().appendTo(this.li);
                b.append(this.file);
            ...
            var a = this.settings, b = this.AU.formatSize(this.size);
            ...
            a.bootstrap && (this.li.addClass("media thumbnail label-info"), this.prevContainer.addClass("pull-left"), this.prevImage.addClass("img-rounded media-object"), this.details.addClass("label label-info").css({"border-bottom-left-radius": 0}),
                this.progressInfo.addClass("progress progress-striped active").css({"margin-bottom": 0}), this.progressBar.addClass("bar"), this.buttons.css({
            ...
            if (this.AU.settings.previews && this.AU.hasAjaxUpload && this.file.type.match(/image.*/) && ("jpg" == this.ext || "gif" == this.ext || "png" == this.ext) && "undefined" !== typeof FileReader) {
                ...
                var e = this.prevImage, d = new FileReader;
                d.onload = function (d) {
                    e.css("cursor", "pointer").attr("src", d.target.result).click(function () {
                        var e =
                            new Image;
                        e.onload = function () {
                            var e = Math.min(c(window).width() / this.width, (c(window).height() - 100) / this.height), h = 1 > e ? this.width * e : this.width, e = 1 > e ? this.height * e : this.height, f = c(window).scrollTop() - 20 + (c(window).height() - e) / 2, g = (c(window).width() - h) / 2, f = c("#ax-box").css({
                                top: f,
                                height: e,
                                width: h,
                                left: g
                            });
                            f.children("img").attr({width: h, height: e, src: d.target.result});
            ...
            var e = c('<div class="ax-ask-inner"><div class="ax-ask-quest">' + b + "</div> </div>").appendTo(this.askDiv), d = c('<a title="Yes" class="ax-button ax-ask-yes"><span class="ax-upload-icon ax-icon"></span> <span>Yes</span></a>').appendTo(e),
        ...
        checkFileExists: function (a) {
            ...
            var a = this.settings, b = this.file, e = this.currentByte, d = this.name, c = this.size, h = a.chunkSize, f = h + e, g = 0 >= c - f, m = b, l = 0 != h ? f / h : 1;
            ...
            this.xhr.onreadystatechange = function () {
                if (4 == this.readyState && 200 == this.status)try {
                    ...
                    if (g)n.onFinishUpload(a.name, a.size, a.status, a.info); else n.currentByte = f, n.uploadAjax()
                } catch (b) {
                    n.onError(b)
                }
            };
            ...
            this.xhr && (this.xhr.abort(), this.xhr = null); else if (this.AU.hasFlash)this.AU.flashObj.stopUpload(this.pos); else {
    ...
    t.prototype = {
        preCheckSettings: function (a) {
            ...
            var e = "ax-browse-c ax-button", b = "ax-upload-all ax-button", d = "ax-clear ax-button", g = "ax-plus-icon ax-icon", h = "ax-upload-icon ax-icon", p = "ax-clear-icon ax-icon";
            ...
            var e = a.split(".").pop().toLowerCase(),
                d = !!(this.files.length < this.settings.maxFiles), l = !!(0 <= c.inArray(e, this.settings.allowExt) || 0 == this.settings.allowExt.length), h = !!(b <= this.max_size), p = "function" === typeof this.settings.validateFile ? this.settings.validateFile.call(this, a, e, b) : "", k = "";
            d || (k = k + f("Max files number reached") + ":" + d + "\n");
            l || (k = k + f("You can upload only files with extension .jpeg and .png.") + "\n");
            h || (k = k + f("File size should not exceed 5 MB.") + "\n");
            "" != p && p !== g && null !== p && (k += p);
            return k
            ...
            this.upload_all = !0;
            if (!1 !== this.settings.beforeUploadAll.call(this, this.files))for (var a = this.getPendingFiles(), b = 0; b < a.length; b++)a[b].startUpload()
        }, clearQueue: function () {
            for (; 0 < this.files.length;)this.removeFile(0)
        }, getParams: function (a, b, c) {
            var d = this.settings, f = "function" == typeof d.remotePath ? d.remotePath() : d.remotePath, h = [];
            ...
            if ("object" == typeof a)for (var g in a)h.push(g + "=" + (c ? encodeURIComponent(a[g]) : a[g])); else if ("string" == typeof a && "" != a)for (c = a.split("&"), g = 0; g < c.length; g++)h.push(c[g]);
            return h
            ...
            var g = Math.round(a), b = Math.pow(10, b);
            a = Math.round(a * b % b);
            return g + "." + a + " " + c[d]
        }, options: function (a, b) {
            if (b !== g && null !== b)this.settings[a] = b, "enable" == a && this.enable(b); else return this.settings[a]
        },
        enable: function (a) {
    ...
    var u = {
        ...
        fileInfo: function (a) {
            var b = "", c;
            for (c in a)a.hasOwnProperty(c) && (b = "object" == typeof a[c] ? b + (c + " : [" + a[c].length + " values]\r\n") : b + (c + " : " + a[c] + "\r\n"));
            alert(b)
        },
        allowDelete: !1
    }, s = {
        init: function (a) {
            return this.each(function () {
                var b = c.extend({}, u, a), e = c(this).html(""), d = e.data("AU");
    ...
    c.fn.ajaxupload = function (a, b) {
        ...
        if ("object" !== typeof a && a)c.error("Method " + a + " does not exist on jQuery.AjaxUploader"); else return s.init.apply(this, arguments)
    }
})(jQuery);